...
export function normalizeHostawayReview(review: HostawayReview): NormalizedReview {
  ...
  if (review.cleanlinessRating) categories.push({ category: 'cleanliness', rating: review.cleanlinessRating });
  if (review.communicationRating) categories.push({ category: 'communication', rating: review.communicationRating });
  if (review.checkInRating) categories.push({ category: 'checkIn', rating: review.checkInRating });
  if (review.accuracyRating) categories.push({ category: 'accuracy', rating: review.accuracyRating });
  if (review.locationRating) categories.push({ category: 'location', rating: review.locationRating });
  if (review.valueRating) categories.push({ category: 'value', rating: review.valueRating });
